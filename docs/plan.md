...
### Phase 16 - Commit 6: Previously-Skipped Performance Items (COMPLETE) ✅
All 8 items that were skipped in earlier commits are now implemented with TDD, code review, and race-tested:

- ✅ **P3: Wire RingBufferSize/SpillThresholdPercent from EngineConfig** — Changed `QueueRing.ringBuffer` from fixed array `[DefaultRingBufferSize]*protocol.Message` to slice `[]*protocol.Message` with per-queue `ringMask`. Added `ringBufferSize`, `ringBufferMask`, `spillThreshold` fields to `DisruptorStorage`, wired from `EngineConfig` in `NewDisruptorStorageWithEngineConfig` with power-of-2 validation. Updated `UnifiedStore` for dynamic sizing. Spill threshold uses integer arithmetic. 4 tests (config wiring, mask works, defaults, spill behavior).
- ✅ **P5: Pool ContentHeader.Serialize()** — Replaced `var result []byte` + 5× `make([]byte, N)` + `append(result, tmp...)` with exact pre-allocation + `binary.BigEndian.AppendUint16/64`. Eliminated DeliveryMode/Priority temp slices. Pre-encodes headers field-table for exact capacity. Zero slice regrowth guaranteed. 1 alloc (result only) vs 2-3 before. 5 tests (correctness, golden bytes, allocs).
- ✅ **P9: Pool WAL done channels** — Added `sync.Pool` for `chan error` (cap 1) in `wal_manager.go`. `Write()` gets from pool with `defer putDoneChannel()`. Defensive drain in `putDoneChannel` prevents deadlock if future code returns channel without reading. 4 tests (pool reuse, allocs, concurrent, WAL integration).
- ✅ **M1: Replace reflect.Select with fan-in channel** — Each consumer gets a forwarding goroutine that sends `*protocol.Delivery` directly (no wrapper struct) to a shared buffered fan-in channel. Main loop does O(1) `select` on fanIn + timeout. Consumer removal detected via active-tags set in discovery loop — stale consumers' forwarders are stopped. `stopAllForwarders()` deferred to prevent goroutine leaks. 4 tests (multi-consumer, goroutine leak, consumer removal + tag reuse, no reflect import).
- ✅ **M2: Batch segment ACK marking** — Added `ackChan` (buffered, cap 1000) and `batchAckLoop` goroutine to `QueueSegments`. `Acknowledge()` sends to channel (non-blocking, falls back to direct `applyAck` if full). Batch loop collects ACKs, applies bitmap in single `bitmapMutex.Lock()`, then calls `acknowledgeInSegment` per offset. Ticker every 10ms. Drains on exit. 1 test + updated 4 existing tests for async ACK processing.
- ✅ **M4: Use fdatasync instead of Sync on Linux** — Platform-specific `fdatasyncFile()`: `syscall.Fdatasync` on Linux (build tag), `f.Sync()` fallback on other platforms. Replaced on 2 hot paths: WAL batch fsync and segment checkpoint sync. Cold paths (close, seal, compaction, index) keep `Sync()` for full metadata durability. 2 tests.
- ✅ **M5: Batch segment writes during checkpoint** — `CheckpointBatch()` now calls `writeMessageBatch()`: single `qs.mutex.Lock()`, serializes all messages into one buffer, single `file.Write()`, bulk index update, bulk metadata update. `sealSegment()` now returns error (was void, silently ignored sync failures) — propagated through `writeMessageBatch` → `CheckpointBatch`. 2 tests (batch write, rollover during batch).
- ✅ **M10: Pool BasicDeliverMethod.Serialize()** — Replaced `var result []byte` + `make([]byte, 8)` for deliveryTag with `make([]byte, 0, 777)` pre-allocation + `binary.BigEndian.AppendUint64`. No temp allocation for delivery tag. 1 alloc (result only) vs 2 before. 3 tests (correctness, golden bytes, allocs).
