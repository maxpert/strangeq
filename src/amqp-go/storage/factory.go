...
func (c *CompositeStorage) GetStorageStats() (*interfaces.StorageStats, error) {
	...
	if statsProvider, ok := c.MessageStore.(interface {
		GetStats() (*interfaces.StorageStats, error)
	}); ok {
	...
	if statsProvider, ok := c.MetadataStore.(interface {
		GetMetadataStats() (*interfaces.MetadataStats, error)
	}); ok {
